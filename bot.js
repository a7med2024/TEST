...
`)
   ...
   message.author.sendEmbed(embed).catch(error => message.reply('😢 Your DM’s is CLosed | خاصك مغلق 😢'))
}   
});
...
『clock /  يعرفك الساعه بصوره مميزه』
『!tickle /  لدغدغه المستخدم المحدد』
『!poke/    لنكز المستخدم المحدد』
『!pat / للمس الشخص المحدد بسرعه 』
...
『لاختيار لون / لون』
『قآيمه الالوان / الوان』
『!bc / لارسال برودكاست بامبد او بدون』
『!obc / برودكست للكل 』
『!paint/يجبلك الكلام الي تكتبه في صوره』
『!trans/يترجم الكلمه الي تكتبه ال اي لغه مع تحديد』
『!new / لفتح تكت للسيرفر』
『تقديم  / للتقديم ع رتبه !』
『!room1 / لانشاء روم التقديم 』
『قبول / رفض // للقبول او الرفض! 』
『!room2 / لانشاء روم الرفض / قبول 』
...
『!clr <numbr> / البوت يمسح الرسال برقم حد اقصي 100 رساله』
...
『!hide / لقفل الشات علي الاعضاء في السيرفر』
『!show / لفتح الشات لجميع الاعضاء في السيرفر』
『!clear  / البوت يمسح كل الشات』
『!cc1 / لانشاء الوان 50 لون منظم』
『!dcolors / لمسح الوان السيرفر』
...
『!talk/للكتابه في روم معين عن طريق البوت 』
   `,`
        ***__Music orders__***
**
...
client.on('message',  message => {
  if(bannedwords.some(word => message.content.includes(word))) {
    message.delete()
    message.reply(" **:angry:احترم نفسك , يمنج الشتايم تمامنا هنا** ").then(msg => {msg.delete(5000)});;
  };
});
...
client.on('typingStart', (ch, user) => {
    if(user.presence.status === 'offline') {
        ...
        ch.send(`${user} **برجاء  حط اون لاين** `)
        .then(msg => {
            msg.delete(10000)
        })
    }
...
const pics = JSON.parse(fs.readFileSync('./pics.json' , 'utf8'));
 client.on('message', message => {
  ...
  if(message.content.startsWith(prefix + "togglepic")) {
          if (!message.channel.guild) return;
...
client.on("message", message => {
  if (message.channel.type === "dm") { //////// A7med codes

      message.channel.startTyping(); //////// A7med codes
      setTimeout(() => { //////// A7med codes
        message.channel.stopTyping(); //////// A7med codes
...
//A7med codes
//جميع الحقوق محفوظة لموقع https ://www.w3schools.com
